 ...
 for c1 in range(numc):
  for c2 in range(numc): # Note that they run up to 0.98
   ...
   if (pbulk[index_b,2]>0.0) and (pjoint[index_c,3]>0.0): # Handle zeros in the probability distribution, avoiding division by zero
    ...
    lambda_ev += -pjoint[index_c,3]*pofa[index_a,1]*math.log(pjoint_o[index_c,3]); # Contribution to the expectation value of lambda, when sampling the bulk
    normfac_lambda_ev += pjoint[index_c,3]*pofa[index_a,1];
   ...
   # sys.exit(1)
    ...
    lambda_ev += -pjoint[index_c,3]*pofa[index_a,1]*math.log(pofa[index_a,1]); normfac_lambda_ev += pjoint[index_c,3]*pofa[index_a,1];
   else:
