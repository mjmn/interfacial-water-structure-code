  ...
  for(int i=1;i<argc;i+=2)
    {
      ...
      if(strcmp(argv[i],"--traj")==0){Ntraj = atoi(argv[i+1]);match=true;}
      if(strcmp(argv[i],"--dx")==0){dx[2] = atof(argv[i+1]);match=true;} // Desired lattice size, will be slightly adjusted later
      if(strcmp(argv[i],"--lx")==0){L_box[0] = atof(argv[i+1]);match=true;} // Box X-dimension - avg_slice.py
      if(strcmp(argv[i],"--ly")==0){L_box[1] = atof(argv[i+1]);match=true;} // Box Y-dimension - avg_slice.py
      if(strcmp(argv[i],"--lz")==0){L_box[2] = atof(argv[i+1]);match=true;} // Box Z-dimension - avg_slice.py
      if(strcmp(argv[i],"--cgl")==0){cg_l = atof(argv[i+1]);match=true;}
      if(strcmp(argv[i],"--bulk")==0){rho_bulk = atof(argv[i+1]);match=true;}
     // MNiesen; introduced several new input variables to make the program more flexible in dealing with varying systems
      if(strcmp(argv[i],"--startT")==0){firstTraj = atof(argv[i+1]);match=true;} // First trajectory frame to read
      ...
      if(strcmp(argv[i],"--nwater")==0){N_water = atof(argv[i+1]);match=true;} // Number of water MOLECULES - avg_slice.py
...
*/
  ...
  for (int t = firstTraj; t < firstTraj+Tstep; t++)
	...
	cerr << "could not find file: " << file_prefix << " attempting xyz format instead." << endl;
        ...
        traj.open(file_prefix);
        if(!traj){ // Also not XYZ, now it really failed
	  cerr << "Also failed to find: " << file_prefix << endl;
          exit(1);
        }
      ...
      for (int n = 0; n < N_prot; n++)
	{
      ...
      cerr << "loaded in protein coordinates" << endl;
      ...
      for (int n = 0; n < N_water; n++)
	{
      ...
      for (int i = 0; i < 3; i++)
	{
	  solv_min[i] = int(solv_min[i]/dx[i])*dx[i] - 5.0;
	  solv_max[i] = (int(solv_max[i]/dx[i])+ 1)*dx[i] + 5.0;

	  N_grid[i] = int((solv_max[i] - solv_min[i])/dx[i]) + 1;
      ...
      /*for (int i = N_iface; i < N_if_tot; i++)
	{
      ...
      */
      ...
      if (t % 2 == 0)
	{
	  logfile << "Calculation is currently  " << fixed << setprecision(3) << (float)t/(float)Tstep*100 << " % done.\n";
	  logfile.flush();
	}
